...
class FaceDetector:
    def __init__(self):
        # OpenCV HAAR
        self._faceCascade = cv2.CascadeClassifier('Data/Model/FaceDetector/haarcascade_frontalface_default.xml')
        ...
        if DNN == "CAFFE":
            self._modelFile = "Data/Model/FaceDetector/res10_300x300_ssd_iter_140000_fp16.caffemodel"
            self._configFile = "Data/Model/FaceDetector/deploy.prototxt"
            self._net = cv2.dnn.readNetFromCaffe(self._configFile, self._modelFile)
        else:
            self._modelFile = "Data/Model/FaceDetector/opencv_face_detector_uint8.pb"
            self._configFile = "Data/Model/FaceDetector/opencv_face_detector.pbtxt"
        ...
        # DLIB MMOD
        self._dnnFaceDetector = dlib.cnn_face_detection_model_v1("Data/Model/FaceDetector/mmod_human_face_detector.dat")
    ...
    def detectFaceOpenCVHaar(self, frame, inHeight=300, inWidth=0):
        ...
        # print('HaarDetector Faces : {0} '.format(len(bboxes)))
        return [frameOpenCVHaar, len(bboxes)]
    ...
    def detectFaceOpenCVDnn(self, frame):
        ...
        # print('DNNDetector Faces : {0} '.format(len(bboxes)))
    ...
    def detectFaceDlibHog(self, frame, inHeight=300, inWidth=0):
        ...
        # print('HoGDetector Faces : {0} '.format(len(bboxes)))
        return [frameDlibHog, len(bboxes)]
    ...
    def detectFaceDlibMMOD(self, frame, inHeight=300, inWidth=0):
        ...
        for faceRect in faceRects:
            ...
            cv2.rectangle(frameDlibMMOD, (cvRect[0], cvRect[1]), (cvRect[2], cvRect[3]), (0, 255, 0),
                          int(round(frameHeight / 150)), 4)
        # print('MMODDetector Faces : {0} '.format(len(bboxes)))
        return [frameDlibMMOD, len(bboxes)]
